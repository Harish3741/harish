...
> or has been retired.
>
> **D. Pictures.** Two or three, not one. List the paths of any screenshots,
> diagrams or exports already in this repo. If there are none and the project
> has a UI, dashboard, workflow canvas or output document worth showing, tell me
> exactly where to capture it — the URL, the file, or the command to run. If you
> can produce a screenshot or render a diagram yourself, do it and give me the
> path. Give each one a sentence saying what I'm looking at.
>
...
Worth knowing: the entry can go in before the pictures do. A path with no file
behind it draws the dashed *photo goes here* slot rather than breaking, so
write-ups and screenshots can arrive separately.
...
| `events` | a row of tabs; each takes `date`, `description`, `highlights`, `images` |
