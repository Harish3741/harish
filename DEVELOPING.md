# Developing this museum

Everything about adding work to the museum and changing how it looks. The short
version of what it is lives in [README.md](README.md).
...
The atrium holds the house rules, lying open on a lectern at the foot of the
compass. The résumé used to hang on the wall above it and came down while it is
blank — `drawNotice` in `art.js` still draws it if it goes back up.
